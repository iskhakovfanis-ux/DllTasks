...
type
  ...
  ƒобавление библиотеки в список
  ќтображение списка загруженных библиотек
  ѕолучение списка методов библиотеки

  ќтображение списка запущенных задач
  ќтображение списка выполненных задач
  ќстановка задачи
  ќтображение информации по задаче (состо€ние, лог, результат)


  ќдновлеменно не может быть выбрано Method и Task.
  ≈сли пользователь выбирает Method, то показываетс€ детали метода
  ≈сли пользователь выбирает Task, то показываютс€ детали задачи

   огда пользователь запускает задачу (со страницы метода), то происходит автоматический переход на вкладку
  задач и выбираетс€ задача (запущенна€)
  ћожно показывать задачу только если установилось состо€ние Working

  DllList мен€етс€ только при загрузке новой библиотеки
  MethodList мен€етс€ при выборе библиотеки
  TaskList мен€етс€ при изменении состо€ни€ задачи
  TaskDetails мен€етс€ при изменении состо€ни€ задачи, либо при изменении лога
...
begin
  ...
  begin
    if (not LoadLibrary(odLoadLibrary.FileName)) then
      ShowMessage('Ќе удалось загрузить библиотеку');
  end;
end;
...
begin
  // —инхронизируем в главном потоке Vcl
  TThread.Queue(nil,
  procedure
  begin
...
begin
  ...
  // ≈сли элемент не найден в текущем списке, то обновл€ем список
  if (TmpItemId < 0) then
    UpdateTaskList()
  else
